...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, key_value, start_pos, rel_pos, incremental_state=None):
        B, N, C = x.shape
...
class CrossBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, num_kv_heads, mlp_ratio=4.0, proj_drop=0., attn_drop=0., **block_kwargs):
        super().__init__()
        self.norm1 = RMSNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.attn = CrossAttention(hidden_size, num_heads=num_heads, num_kv_heads=num_kv_heads, qkv_bias=False, proj_drop=proj_drop, attn_drop=attn_drop, **block_kwargs)
        self.norm2 = RMSNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        mlp_hidden_dim = int(hidden_size * mlp_ratio * 2 / 3 / 64) * 64
        self.mlp = SwiGLU(hidden_size, mlp_hidden_dim, drop=proj_drop)
    ...
    def forward(self, x, c, key_value, start_pos, rel_pos, incremental_state=None):
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=-1)
        x = x + gate_msa * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), key_value, start_pos, rel_pos, incremental_state)
...
class YOCO(nn.Module):
    def __init__(
        ...
        self.diffusion_blocks = nn.ModuleList([
            CrossBlock(hidden_size, self.num_heads, self.num_kv_heads, mlp_ratio=mlp_ratio, proj_drop=drop, attn_drop=drop) for _ in range(diffusion_depth)
        ])
        self.key_proj = nn.Linear(hidden_size, hidden_size // self.num_heads * self.num_kv_heads, bias=False)
        self.value_proj = nn.Linear(hidden_size, hidden_size // self.num_heads * self.num_kv_heads, bias=False)
    ...
    def forward_diffusion(self, x, t, condition):
        ...
        key, value = self.key_proj(condition), self.value_proj(condition)
        key = key.view(bsz, seq_len, self.num_kv_heads, -1)
        value = value.view(bsz, seq_len, self.num_kv_heads, -1)
        rel_pos = self.build_rel_pos(condition)
        key = apply_rotary_emb(key, *rel_pos, interleaved=True)
        key_value = (key, value)
        x = self.noisy_x_embedder(x)
        
        for block in self.diffusion_blocks:
            x = block(x, c=t, key_value=key_value, start_pos=0, rel_pos=rel_pos)
    ...
    def forward_with_cfg(self, x, t, condition, cfg_scale):
        """
        Forward pass of ClassYOCO, but also batches the unconditional forward pass for classifier-free guidance.
        """
...
def YOCO_13B(**kwargs):
    return YOCO(depth=40, hidden_size=5120, num_heads=40, mlp_ratio=6, **kwargs)

def YOCO_7B(**kwargs):
    return YOCO(depth=32, hidden_size=4096, num_heads=32, mlp_ratio=6, **kwargs)

def YOCO_3B(**kwargs):
    return YOCO(depth=32, hidden_size=2560, num_heads=20, mlp_ratio=6, **kwargs)

def YOCO_XL(**kwargs):
    return YOCO(depth=24, hidden_size=2048, num_heads=16, mlp_ratio=6, **kwargs)

def YOCO_Large(**kwargs):
    return YOCO(depth=24, hidden_size=1536, num_heads=12, mlp_ratio=6, **kwargs)

def YOCO_Medium(**kwargs):
    return YOCO(depth=24, hidden_size=1024, num_heads=16, mlp_ratio=6, **kwargs)

def YOCO_Base(**kwargs):
    return YOCO(depth=12, hidden_size=768, num_heads=12, mlp_ratio=6, **kwargs)

def YOCO_H(**kwargs):
    return YOCO(depth=40, hidden_size=1280, num_heads=20, mlp_ratio=4, diffusion_depth=12, **kwargs)

def YOCO_L(**kwargs):
    return YOCO(depth=32, hidden_size=1024, num_heads=16, mlp_ratio=4, diffusion_depth=8, **kwargs)

def YOCO_B(**kwargs):
    return YOCO(depth=24, hidden_size=768, num_heads=12, mlp_ratio=4, diffusion_depth=6, **kwargs)

YOCO_models = {
    'YOCO-13B': YOCO_13B, 'YOCO-7B': YOCO_7B, 'YOCO-3B': YOCO_3B, 'YOCO-XL': YOCO_XL, 'YOCO-Large': YOCO_Large, 'YOCO-Medium':  YOCO_Medium, 'YOCO-Base': YOCO_Base,
    'YOCO-H': YOCO_H, 'YOCO-L': YOCO_L, 'YOCO-B': YOCO_B
}