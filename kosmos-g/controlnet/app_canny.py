...
def create_demo_canny(generation_fn):
    with gr.Blocks() as demo:
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Accordion("Advanced options", open=False):
                    ...
                    canny_low_threshold = gr.Slider(label='Canny low threshold', minimum=1, maximum=255, value=100,
                                                    step=1)
                    canny_high_threshold = gr.Slider(label='Canny high threshold', minimum=1, maximum=255, value=200,
                                                     step=1)
        ...
        ips = [prompt, num_inference_steps, text_guidance_scale, negative_prompt, num_images_per_prompt, image,
               image_resolution, canny_low_threshold, canny_high_threshold, *input_images]
