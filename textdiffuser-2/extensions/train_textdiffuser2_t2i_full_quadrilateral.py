    ...
    def preprocess_train(examples):            
        ...
        for image in examples[image_column]:
            ...
            for line in ocrs:
                ...
                ocrs_temp.append([x1, y1, x2, y2, x3, y3, x4, y4, pred])
            ...
            for line in ocrs_temp:
                ...
                x1, y1, x2, y2, x3, y3, x4, y4, pred = line
                x1 = x1 // (512 // args.granularity)
                x2 = x2 // (512 // args.granularity)
                x3 = x3 // (512 // args.granularity)
                x4 = x4 // (512 // args.granularity)
                y1 = y1 // (512 // args.granularity)
                y2 = y2 // (512 // args.granularity)
                y3 = y3 // (512 // args.granularity)
                y4 = y4 // (512 // args.granularity)
                # ocr_ids.extend(['l'+str(x_left), 't'+str(y_top), 'r'+str(x_right), 'b'+str(y_bottom)])

                #### simply using l/t to indicate the first/second coords is fine
                ocr_ids.extend(
                    [
                        'l'+str(x1), 't'+str(y1), 
                        'l'+str(x2), 't'+str(y2), 
